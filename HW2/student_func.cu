                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for(int c=0; c<numCols; c++) {
    float result = 0.f;
    for (int delta_r = -filterWidth/2; delta_r <= filterWidth/2; ++delta_r)
    for (int delta_c = -filterWidth/2; delta_c <= filterWidth/2; ++delta_c) {
      int image_r = min(max(r + delta_r, 0), static_cast<int>(numRows - 1));
      int image_c = min(max(c + delta_c, 0), static_cast<int>(numCols - 1));
    
      float image_value = inputChannel[image_r * numCols + image_c];
      float filter_value = filter[(delta_r + filterWidth/2) * filterWidth + delta_c + filterWidth/2];
                       ...
                       int numCols)
{
  ...
  */
  ...
  for(int c=0; c<numCols; c++) {
    int i = r*numCols + c;
    unsigned char red = redChannel[i];
    unsigned char green = greenChannel[i];
    unsigned char blue = blueChannel[i];
    uchar4 outputPixel = make_uchar4(red, green, blue, 255);    
    outputImageRGBA[i] = outputPixel;
  }
}
